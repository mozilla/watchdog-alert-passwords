...
const SyncUtils = Cu.import("resource://services-sync/util.js").Utils;

// const notifBox = (function() {
...
var PopupAlertsManager = {
    displayAlertBar: function(alert) {
        ...
        notifBox.appendNotification(AlertManager.getAlertString(alert),"watchdog-alerts",null,notifBox.PRIORITY_WARNING_HIGH,[
            {
                accessKey: 'o',
                callback: function(){},
                label: 'Okay',
...
var AlertManager = function() {
    ...
    const ALERTS_FILENAME = 'watchdog/password_alerts';
    ...
    return {
        ...
        readAlerts: function() {
            var cryptoService = Cc["@mozilla.org/login-manager/crypto/SDR;1"]
                        .getService(Ci.nsILoginManagerCrypto);
            try {
                // if (simpleStorage.storage['encryptedAlerts'] == undefined)
                //     throw "No alerts stored";
                
                SyncUtils.jsonLoad(ALERTS_FILENAME,this,function(obj) {                    
                    // May prompt for master password, throws exception if user hits cancel.
                    var decryptedAlerts = cryptoService.decrypt(obj.ciphertext);
                    alerts = JSON.parse(decryptedAlerts);
                });
            }
            catch (e) {
                console.log("Could not read alerts: " + e);
                alerts = [];
            }
        },
        update: function() {
        ...
        writeAlerts: function() {
            ...
            // FIXME: Move this over to Jetpack simple-storage, which isn't working for me for some reason.
            var cryptoService = Cc["@mozilla.org/login-manager/crypto/SDR;1"]
                        .getService(Ci.nsILoginManagerCrypto);
            try {
                // May prompt for master password, throws exception if user hits cancel.    
                var encryptedAlerts = cryptoService.encrypt(JSON.stringify(alerts));
                // simpleStorage.storage['encryptedAlerts'] = encryptedAlerts.toString();
                SyncUtils.jsonSave(ALERTS_FILENAME,this,{
                    "ciphertext": encryptedAlerts
                });
            }
            catch(e) {
                console.log("Could not encrypt alerts!");
            }
        }
    };    
}();
...
win.setTimeout(function() { AlertManager.loadAlertsFromSS(); },500);
