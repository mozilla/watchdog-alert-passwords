...
var secureLoginChecker = {
    ...
    /* ---------- private memebers ---------- */
    _sessionIgnore: {},  // once a user closes a notificationbox, we don't show it again this session
    ...
    _newDocument: function(aWindow) {
      // XXX refactor this perhaps
        ...
        var host = uri.host;
        // if the user closed this notification, but didn't ignore, lets not bother
        // them with every page.  We'll notify them again in 24 hours or the next
        // browser session
        if (this._sessionIgnore[host]) {
          if (Date.now() - this._sessionIgnore[host] < 86400000) {
            //console.log("notification for "+host+" ignored rest of this session");
            return;
          }
          delete this._sessionIgnore[host];
        }
        ...
        let buttons = [
            ...
            accessKey: 'm',
            callback: function() {
                xulWindow.openUILinkIn("about:passwords#"+passwordManager.baseDomain(host), "tab", null, null, null);;
            }
        },
        {
        ...
        if (!ignore)
          nBox.appendNotification(msg, "watchdog-password-notification",
                                  ...
                                  level, buttons,
                                  function cb() {
                                    self._sessionIgnore[host] = Date.now();
                                    //console.log("watchdog-password-notification: "+JSON.stringify(arguments))
                                  });
    ...
    watchForAccountSetup: function(xulWindow, browser) {
      ...
      browser.addEventListener('load', function(event) {
        ...
        for (var i=0; i < document.forms.length; i++) {
          ...
          var pwFields = self.getPasswordFields(document.forms[i], false);
    ...
    installPasswordHandlers: function(xulWindow, browser, form, pwFields) {
        ...
        try {
          var ignore = this._prefBranch.getBoolPref(host+".nohelp");
          if (ignore) return;
        } catch(e) {}
        ...
        field.element.addEventListener('focus', function () {
          let buttons = [
              ...
              callback: function() {
                ...
                passwordGenerator.showPopup("watchdog-password-generate-icon", msg);
              }
          },
          {
              ...
              accessKey: 'n',
              callback: function() {
                self._prefBranch.setBoolPref(host+".nohelp", true);
              }
          }];
