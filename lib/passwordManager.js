...
function getSimiliarities(loginsTable) {
    ...
    for (var password1 in loginsTable) {
        for (var password2 in loginsTable) {
            ...
            if (exports.passwordSimilarityCheck(password1,password2)) {
                loginsTable[password1].similarTo.push.apply(loginsTable[password1].similarTo, loginsTable[password2].shared);
                loginsTable[password2].similarTo.push.apply(loginsTable[password2].similarTo, loginsTable[password1].shared);
            }
        }
        passwordsChecked[password1] = true;
    }    
...
exports.getLoginMetaInfo = function getLoginMetaInfo() {
    ...
    for (let login in logins) {
        ...
        try {
            ...
            host = /(\w+)\.\w+$/.exec(url.URL(li.hostname).host)[1];
        }
        catch (e) {
            // These might not all be valid URLs, e.g. chrome://...
            // So if the URL class throws an error, just use the hostname again.
            try {
                host = /(\w+)\.\w+$/.exec(li.hostname)[1];
            } catch(e) {
        ...
        var unsecure = li.hostname.indexOf("http://") === 0;
        if (unsecure) {
        ...
        if (!pwInfo[li.password]) {
            ...
            pwInfo[li.password].shared.push(li.hostname);
            if (unsecure) pwInfo[li.password].unsecure = true;
    ...
    for (let login in logins) {
        ...
        var info = {
            ...
            usedBy: pwInfo[li.password].shared,
            similarTo: pwInfo[li.password].similarTo,
            age: age,
            unsecure: pwInfo[li.password].unsecure,
        };
    ...
    let dup = 100 - (scores.duplicates / scores.logins) * 100,
        sim = 100 - (scores.similarity / scores.logins) * 100,
        age = 100 - (scores.over_ninty / scores.logins) * 100;
    console.log("str: "+Math.round(scores.avg_strength * .6)+" dup: "+dup+":"+Math.round(dup * .2)+" sim: "+sim+":"+Math.round(sim * .05)+" age:"+age+":"+Math.round(age * .05))
