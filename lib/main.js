...
function windowInit(win) {
  ...
  let uri = require("sdk/self").data.url("css/global-overlay.css");
  ...
  icon = document.createElementNS(XUL_NS, "image");
  icon.setAttribute('id', "watchdog-password-generate-icon");
  icon.setAttribute('class', "notification-anchor-icon");
  icon.setAttribute('role', "button");
  noticons.appendChild(icon);
}
